module.exports = {
  "Search for a foreign word (non-english word)": function(browser) {
    browser
    // Navigate to duckduckgo
      .url("http://duckduckgo.com")
      // Enter "instabug" into the search box
      .setValue("#search_form_input_homepage", "محمد صلاح")
      // Click enter
      ...
      browser.elements("css selector", '#links > div.results_links_deep', function(text){
        ...
        array.forEach(function(currentValue,index){
          ...
          browser.expect.element(browser.page.searchResults().getLinksByIndex(index + 1)).text.to.contain("محمد صلاح"); //assert each result contains the word 'instabug' regardless of whether it's lower or upper case
        });
      });
      // Close the browser
      browser.end();
